...
<cat id="categoryBar">
  <ul>
    ...
    <body>
        <?php  
            $itemInfo = "SELECT ItemID, ItemName, ItemCategory, ItemCondition, ItemDescription, PictureName, ItemPrice, UserHomeTown, UserFName, UserEmail, CellNumber FROM items JOIN user USING(UserID) JOIN picture USING(ItemID) WHERE ItemID LIKE '%".$ItemID."%'";
            ...
            if ($result->num_rows > 0) {
                ...
                $row = $result->fetch_assoc();
                  echo("
                    ...
                    <div class=\"itemViewPicture\">  
                    <div class=\"bodyStyle\"><p> Home > ".$row["ItemCategory"]." > item ID ".$row["ItemID"]."</p></div>
                    <p><h1>" .$row["ItemName"]."</h1></p>
                    <p><h2 class = \"addPrice\">"."R " .$row["ItemPrice"]."</h2></br></p>
                    <span><img src= Pictures\\". $row["PictureName"]." alt=\"Picture of item\" width=\"450\" height=\"auto\"></span>
                    </br></br>
                    <p><h3>General information: </h3></p></br>
                    <p> Location: ".$row["UserHomeTown"]."</p>
                    <p> Category: ".$row["ItemCategory"]."</p>
                    <p> Condition: ".$row["ItemCondition"]."</p>
                    </br></br>
                    <p><h3>Description: </h3></p></br>
                    <p>".$row["ItemDescription"]."</p>
                    </br></br>
                    <p><h3>Seller Details: </h3></p></br>
                    <p> First Name: ".$row["UserFName"]."</p>
                    <p> Email: ".$row["UserEmail"]."</p>
                    <p> Cell Number: ".$row["CellNumber"]."</p>
                    </div>
                  ");
              } else {
                echo "0 results";
